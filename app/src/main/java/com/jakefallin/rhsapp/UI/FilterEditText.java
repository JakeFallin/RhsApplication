...
public class FilterEditText extends EditText {
    ...
    public FilterEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray attr = context.obtainStyledAttributes(attrs, R.styleable.MinMaxFilter);
        double min = attr.getFloat(R.styleable.MinMaxFilter_min, 1);
        double max = attr.getFloat(R.styleable.MinMaxFilter_max, 9999);
        setFilters(new InputFilter[]{new MinMaxInputFilter(min, max)});
        attr.recycle();
    }
